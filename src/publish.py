...
def check_article_integrity(markdown: str = None, html: str = None, 
    ...
    if draft_content is not None:
        ...
        # 检查外部 URL（已上传到微信）
        # 注意：微信可能使用 data-src 而非 src
        external_images = re.findall(r'<img src="(http[^\"]+)"', draft_content)
        data_src_images = re.findall(r'<img[^>]*data-src="(http[^\"]+)"', draft_content)
        all_external = external_images + data_src_images
...
def find_cover_image(theme: str) -> Optional[str]:
    ...
    if cache_dir.exists():
        covers = list(cache_dir.glob("cover_*.jpg"))
        if covers:
...
def publish_article(args, components) -> bool:
    ...
    # 6. 上传封面图
    logging.info("上传封面图...")
    result = components['material'].upload_image(cover_path)
